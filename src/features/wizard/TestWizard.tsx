...
export default function TestWizard({ esPrueba = false }: { esPrueba?: boolean }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg)] flex flex-col relative overflow-hidden">
      {/* Background carousel — visible through a dark veil */}
      <div className="absolute inset-0 z-0">
        <BackgroundCarousel slides={backgroundSlides} intervalMs={6500} />
        {/* Dark veil — imagery stays alive, text stays readable */}
        <div className="absolute inset-0 bg-[var(--color-bg)]/55" />
        <div className="absolute inset-0 bg-gradient-to-b from-[var(--color-bg)]/70 via-transparent to-[var(--color-bg)]/75" />
      </div>
      ...
      <div className="sticky top-0 z-40 glass">
        <div className="w-full px-6 py-4 relative">
          <div className="flex items-start gap-4">
            ...
            <button
              onClick={toggleAudio}
              className="flex items-center justify-center w-8 h-8 shrink-0 mt-0.5 text-[var(--color-text-secondary)] hover:text-[var(--color-neon-secondary)] transition-colors"
        ...
        <div className="w-full max-w-4xl">
          ...
          {isDisclaimer && (
            <div className="space-y-6">
              <div className="neon-border rounded-3xl p-6 md:p-9 space-y-5 bg-[var(--color-surface)]/80 backdrop-blur-xl shadow-[0_8px_40px_rgba(0,51,165,0.10)]">
                <h2 className="text-2xl md:text-3xl font-bold text-[var(--color-text-primary)] flex items-center gap-3">
                  <span className="w-12 h-12 rounded-xl bg-[linear-gradient(135deg,var(--color-neon-primary),var(--color-neon-secondary))] flex items-center justify-center text-xl shadow-[0_0_16px_color-mix(in_srgb,var(--color-neon-primary)_30%,transparent)]">
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl font-bold transition-transform duration-150 will-change-transform ${
                    canGoNext
                      ? "text-[var(--color-text-primary)] bg-[linear-gradient(135deg,var(--color-neon-primary),var(--color-neon-secondary))] hover:scale-[1.02] active:scale-[0.98] shadow-[0_0_24px_color-mix(in_srgb,var(--color-neon-primary)_35%,transparent)]"
